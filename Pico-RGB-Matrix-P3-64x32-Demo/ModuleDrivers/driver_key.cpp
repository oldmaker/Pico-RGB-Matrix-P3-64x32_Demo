...
/**********************************************************
  ...
  * Function description: Determine whether the button is 
...
void WhichKeyPress(void)
{
  ...
  else
  {
    if(memuKey > 20 && memuKey < 500)
   {
      ...
      event.iPressure = memuKey; /*Pressure value*/
      PutInputEvent(&event);
    }
  ...
  else
  {
    if(downKey > 20 && downKey < 500){
      ...
      event.iPressure = downKey; /*Pressure value*/
      PutInputEvent(&event);
    }
    downKey = 0;
  }
  ...
  {
    if(upKey > 20 && upKey < 4000)
    {
      event.eType = INPUT_EVENT_TYPE_KEY;
      event.iKey  = KEY2_UP;
      event.iPressure = upKey; /*Pressure value*/
      PutInputEvent(&event);
    }
    upKey = 0;
  }
